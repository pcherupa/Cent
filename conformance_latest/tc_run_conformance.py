...
def run_tests(tests) :
  ...
  for test in tests:
   ...
   subtest_name = string.replace(test_name, os.sep, '/')
   ...
   print("##teamcity[testStarted name='" + subtest_name + "' captureStandardOutput='true']")
   log_file.write("     ----------------------------------------------------------------------------------------\n")
   ...
   if (result == 0):
    ...
    print("##teamcity[testFailed name='" + subtest_name + "']")

   print("##teamcity[testFinished name='" + subtest_name + "']")
...
try:
 log_file = sys.stdout #open(log_file_name, "w")
except IOError:
 print "Could not open log file " + log_file_name

variant="conformance"
match = re.search("opencl_conformance_tests_(\S+)\.csv", sys.argv[1])
if (match):
 variant = match.group(1)
write_screen_log("##teamcity[testSuiteStarted name='conformance']")
write_screen_log("##teamcity[testSuiteStarted name='" + variant + "']")
...
write_screen_log("##teamcity[testSuiteFinished name='" + variant + "']")
write_screen_log("##teamcity[testSuiteFinished name='conformance']")
log_file.close()
